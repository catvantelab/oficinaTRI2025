...
fit.t2 <- readRDS('saidas/fit_t1.rds')
escore.t2 <- readRDS('saidas/escore_t1.rds')
```

O objetivo é montar um instrumento de 40 itens que meça com boa precisão o intervalo entre `-1.5` e `1.5`. Nesta atividade, vamos selecionar os itens de acordo com sua dificuldade, independente do conteúdo.
...
Ao combinar os itens das duas aplicações, conseguimos criar um instrumento otimizado que, conforme mostram os gráficos, agora oferece precisão maior na região mais alta da escala. Porém, ele atende ao objetivo? Caso negativo, o que é preciso fazer para atender ao objetivo?

Pense em outros usos para o escore (e.g., seleção, certificação e ampliação do intervalo medido) e tente montar um instrumento que atenda a esses objetivos.
